...
class _AuthContext:
    ...
    def __init__(self, payload: dict):
        ...
        _raw_sub: str = payload.get("sub") or ""
        ...
        self.email: str | None = payload.get("email") or (_raw_sub if "@" in _raw_sub else None)
        self.username: str | None = (
            payload.get("username")
            or payload.get("preferred_username")
            or (_raw_sub.split("@", 1)[0] if "@" in _raw_sub else None)
        )
        # Prefer email over UUID as the effective identity key so store lookups
        # are stable across IS subject-type changes (uuid vs email).
        self.sub: str = self.email or _raw_sub
