...
DEFAULT_RULES: tuple[KeywordRule, ...] = (
    ...
    # Also accepts common typo variants ("cubical") and onboarding phrases so
    # prompts like "on board ramith allocate cubical and laptop" don't fall
    # through to the IT-only equipment listing rule.
    KeywordRule(
        keywords=(
            "cubicle", "cubicles", "cubical", "cubicals",
            "seat", "seats", "seating",
    ...
    KeywordRule(
        keywords=(
            ...
            "apply annual", "apply sick", "apply personal",
            ...
            "days annual", "days sick", "days personal",
            "annual from", "sick from", "personal from",
        ),
        agent_id="hr_agent",
...
_LEAVE_TYPE_RE = re.compile(r"\b(Annual(?:\s+Leave)?|Sick(?:\s+Leave)?|Personal(?:\s+Leave)?)\b", re.IGNORECASE)
_ISO_DATE_RE = re.compile(r"\b(\d{4}-\d{2}-\d{2})\b")
# Natural-language date: "20th june 2026", "june 20 2026", "20 june 2026"
_NATURAL_DATE_RE = re.compile(
    r"\b(\d{1,2})(?:st|nd|rd|th)?\s+(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|"
    r"may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)"
    r"\s+(\d{4})\b"
...
# "5 days from 20th june" — extract duration in days
_DURATION_DAYS_RE = re.compile(r"\b(\d+)\s+days?\b", re.IGNORECASE)
...
def _extract_inline_args(tool_id: str, message: str) -> dict:
    ...
    if tool_id == "hr.apply_leave":
        ...
        if m_type:
            ...
            if raw.startswith("annual"):
                out["leave_type"] = "Annual Leave"
            elif raw.startswith("sick"):
                out["leave_type"] = "Sick Leave"
            elif raw.startswith("personal"):
                out["leave_type"] = "Personal Leave"

        # Try ISO dates first, then natural-language dates
        ...
        if "start_date" not in out:
            # Parse "20th june 2026" or "june 20 2026"
            parsed_dates = []
            for m in _NATURAL_DATE_RE.finditer(message):
                if m.group(1):  # "20th june 2026"
                    day, month_s, year = int(m.group(1)), m.group(2).lower()[:3], int(m.group(3))
                else:  # "june 20 2026"
                    month_s, day, year = m.group(4).lower()[:3], int(m.group(5)), int(m.group(6))
                month = _MONTH_MAP.get(month_s)
                if month:
                    parsed_dates.append(f"{year}-{month:02d}-{day:02d}")
            if parsed_dates:
                out["start_date"] = parsed_dates[0]
            if len(parsed_dates) >= 2:
                out["end_date"] = parsed_dates[1]

        # If only start_date and a duration ("5 days"), compute end_date
        if "start_date" in out and "end_date" not in out:
            m_dur = _DURATION_DAYS_RE.search(message)
            if m_dur:
                from datetime import date, timedelta
                try:
                    start = date.fromisoformat(out["start_date"])
                    end = start + timedelta(days=int(m_dur.group(1)) - 1)
