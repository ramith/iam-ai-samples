...
from orchestrator.reports.proxy import _can_use_reports_scope, forward_with_token_a
...
def build_reports_router(deps: ReportsRouterDeps) -> APIRouter:
    ...
    async def approve_leave(
        ...
        if not _can_use_reports_scope(session, "hr_approve_rest"):
            token_a = session.token_a
            scope_string = (token_a.scope or "") if token_a is not None else ""
            _logger.warning(
                "reports_approve_preflight_scope_denied | session_id=%s roles=%s",
                session.session_id,
    ...
    async def reject_leave(
        ...
        if not _can_use_reports_scope(session, "hr_approve_rest"):
            ...
            scope_string = (token_a.scope or "") if token_a is not None else ""
            _logger.warning(
                "reports_reject_preflight_scope_denied | session_id=%s roles=%s",
                session.session_id,
                list(session.token_a_groups),
            )
            return JSONResponse(
