...
class GetLeaveBalanceArgs(BaseModel):
    ...
    consulted — the service always keys on ``claims.effective_sub``. Sprint 5 will drop
...
def build_hr_mcp_router(deps: HRMcpToolRouterDeps) -> APIRouter:
    ...
    async def _validate_with_required_scope(
        token_str: str,
        *,
        required_scope: str,
        rid: str,
        ...
        """Validate the token against the exact required scope.

        Authorization is enforced at the resource server boundary using
        IS-issued scopes; there is intentionally no scope downgrade path.
        """
        logger.debug(
            "%s scope_validation rid=%s required=%s",
            tool_name,
            rid,
            required_scope,
        )
        return await deps.validator.validate_token(
            token_str,
            required_scopes=frozenset({required_scope}),
        )

    async def _validate_hr_self_scope(
        token_str: str,
        *,
        rid: str,
        tool_name: str,
    ):
        return await _validate_with_required_scope(
            token_str,
    ...
    async def get_leave_balance(
        ...
        try:
            claims = await _validate_hr_self_scope(
                token_str,
                rid=rid,
        ...
        result = await hr_service.get_my_leave_balance(
            claims.effective_sub, _username_for(claims), ""
        )
    ...
    async def get_leave_history(
        ...
        return GetLeaveHistoryResult(
            employee_id=claims.effective_sub,
    ...
    async def approve_leave(
        ...
        # the human manager (claims.effective_sub). The audit trail records both via
        ...
        result = await hr_service.approve_leave_request(
            ...
            reviewer_sub=claims.effective_sub,
        ...
        if result.get("success"):
            return ApproveLeaveResult(
                ...
                approved_by=act_sub or claims.effective_sub,
            )
    ...
    async def reject_leave(
        ...
        if result.get("success"):
            return RejectLeaveResult(
                ...
                employee=result.get("employee"),
                notification=result.get("notification"),
                rejected_by=act_sub or claims.effective_sub,
            )
        return RejectLeaveResult(
            success=False,
            request_id=body.leave_id,
    ...
    async def apply_leave(
        ...
        logger.debug(
            ...
            rid, claims.effective_sub, body.leave_type, body.start_date, body.end_date,
        )
    ...
    async def get_cubicle_summary(
        ...
        try:
            await _validate_with_required_scope(
                token_str,
                required_scope="hr_read_rest",
                rid=rid,
                tool_name="get_cubicle_summary",
            )
        except (JWTValidationError, PeerTrustError, ScopeError) as exc:
            logger.warning(
    ...
    async def get_vacant_cubicles_on_floor(
        ...
        try:
            await _validate_with_required_scope(
                ...
                required_scope="hr_read_rest",
                rid=rid,
                tool_name="get_vacant_cubicles_on_floor",
            )
        except (JWTValidationError, PeerTrustError, ScopeError) as exc:
            logger.warning(
    ...
    async def get_my_cubicle(
        ...
        result = await hr_service.get_my_cubicle(
            sub=claims.effective_sub, username=getattr(claims, "username", None)
        )
    ...
    async def assign_cubicle(
        ...
        try:
            claims = await _validate_with_required_scope(
                token_str,
                required_scope="hr_assets_write_rest",
                rid=rid,
                tool_name="assign_cubicle",
            )
        except (JWTValidationError, PeerTrustError, ScopeError) as exc:
            logger.warning(
