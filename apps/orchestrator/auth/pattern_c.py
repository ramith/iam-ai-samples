...
from common.auth.jwt_validator import _extract_scopes
...
def _scopes_from_scope_string(scope: str | None) -> list[str]:
    return [part for part in (scope or "").split() if part]


def _scopes_from_unverified_jwt(jwt_token: str | None) -> frozenset[str]:
    if not _looks_like_jwt(jwt_token):
        return frozenset()
    try:
        payload = jwt.decode(  # nosec B113 - unverified decode is intentional for scope normalization
            jwt_token or "",
            options={
...
def _merge_scope_string(response_scope: str | None, jwt_scope_set: frozenset[str]) -> str:
    response_scopes = _scopes_from_scope_string(response_scope)
    response_set = set(response_scopes)
    extras = [s for s in sorted(jwt_scope_set) if s not in response_set]
    merged = response_scopes + extras
    return " ".join(merged)
...
class PatternCExchanger:
    ...
    async def exchange(
        ...
        *,
        ...
        # WSO2 can return opaque access tokens; in that case fall back to
        # validating id_token so login does not fail on token format alone.
        token_for_validation = token_a.access_token
        if not _looks_like_jwt(token_for_validation):
            if _looks_like_jwt(token_a.id_token):
                logger.warning(
                    "pattern_c_access_token_opaque | falling back to id_token validation"
                )
                token_for_validation = token_a.id_token or ""
            else:
                raise JWTValidationError(
                    "Neither access_token nor id_token is a JWT; cannot validate token response"
                )

        claims = await validate(
            token_for_validation,
            self._validator,
        ...
        # Some IS setups emit granted API scopes under JWT claims (`scp` /
        # `permissions`) but keep the top-level /token `scope` field at OIDC
        # defaults. Merge both surfaces so session/UI/preflight checks see the
        # full granted set.
        jwt_scope_set = _scopes_from_unverified_jwt(token_for_validation)
        merged_scope = _merge_scope_string(token_a.scope, jwt_scope_set)
        if merged_scope != (token_a.scope or ""):
            logger.info(
                "pattern_c_scope_normalized | response_scope=%r merged_scope=%r",
                token_a.scope,
                merged_scope,
            )
            token_a = OAuthToken(
                access_token=token_a.access_token,
                token_type=token_a.token_type,
                expires_in=token_a.expires_in,
                expires_at=token_a.expires_at,
                refresh_token=token_a.refresh_token,
                scope=merged_scope,
                id_token=token_a.id_token,
            )
