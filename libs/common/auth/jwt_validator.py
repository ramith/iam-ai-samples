...
def _extract_scopes(payload: dict[str, Any]) -> frozenset[str]:
    """Return a normalized scope set from the common WSO2 scope-bearing claims.

    WSO2 deployments may surface granted scopes under ``scope`` (space-separated
    string), ``scp`` (string or array), or ``permissions`` (string or array).
    This helper normalizes all of them into one deduplicated set so callers that
    enforce scope-based access control do not silently miss granted API scopes.
    """
    raw_values: list[Any] = []
...
def _extract_claim_list(payload: dict[str, Any], *keys: str) -> tuple[str, ...]:
    """Return a normalized tuple of string claims from one or more keys."""
    values: list[str] = []
    for key in keys:
        ...
        if isinstance(value, str):
            # Role/group claims may contain names with spaces (e.g. "HR Admin").
            # Split only on explicit list delimiters when present; otherwise keep
            # the full value intact.
            if "," in value:
...
async def validate(
    ...
    *,
    ...
    return JWTClaims(
        ...
        jti=jti,
        act=payload.get("act"),
        scope=" ".join(sorted(token_scopes)) if token_scopes else payload.get("scope"),
        aut=payload.get("aut"),
