...
_REQUIRED_ARGS: dict[str, list[str]] = {
    ...
    "hr.cubicle_assign": ["cubicle_id", "employee_username"],
}
...
class HRDispatcherDeps:
    ...
    # Optional peer client for hr->it onboarding coordination (non-CIBA).
    # When set, a successful cubicle assignment also fetches it_agent's standard
    # new-hire kit and attaches it to the result as ``it_kit``.
    it_peer_client: "object | None" = None
...
class HRDispatcher:
    ...
    # ── hr->it onboarding coordination (non-CIBA peer call) ───────────────────

    async def _maybe_attach_it_kit(
        self, tool: str, tool_result: dict, request_id: str
    ) -> dict:
        """For a cubicle assignment, attach it_agent's standard new-hire kit.

        Best-effort A2A coordination chain (hr->it): demonstrates sibling-agent
        calling during onboarding. Never blocks or fails the cubicle result —
        a peer error just omits the ``it_kit`` hint. Consent-free: it_agent's
        peer endpoint returns static policy only; the actual asset issuance
        still flows through the orchestrator -> it_agent CIBA path.
        """
        if tool not in ("hr.cubicle_summary", "hr.cubicle_assign"):
            return tool_result
        peer = getattr(self._deps, "it_peer_client", None)
        if peer is None:
            return tool_result
        kit = await peer.fetch_onboard_kit(request_id=request_id)
        if kit and isinstance(tool_result, dict):
            enriched = dict(tool_result)
            enriched["it_kit"] = kit.get("kit", [])
            enriched["it_kit_note"] = kit.get("note", "")
            logger.info("hr_dispatcher_it_kit_attached request_id=%s", request_id)
            return enriched
        return tool_result
    ...
    async def revoke_jti(self, jti: str, user_sub: str, exp: float, reason: str) -> None:
        ...
        if cache_key is not None:
            ...
            if popped is not None:
                # Defence-in-depth: also revoke the OBO token at WSO2 IS so it
                # is dead at the IdP, not just locally denylisted/evicted.
                await self._revoke_obo_at_is(popped.token, reason)
                logger.info(
    ...
    async def _revoke_obo_at_is(self, token, reason: str) -> None:  # type: ignore[no-untyped-def]
        """Best-effort revoke the OBO token (and its refresh token) at WSO2 IS.

        Layered on top of the local denylist so the token is also invalidated
        at the IdP. Never raises — a failure here must not break the logout /
        admin-terminate cascade (the denylist already blocks local reuse).
        """
        try:
            access = getattr(token, "access_token", "") or ""
            refresh = getattr(token, "refresh_token", "") or ""
            if access:
                await self._deps.ciba_client.revoke_token(
                    oauth_client_id=self._deps.oauth_client_id,
                    oauth_client_secret=self._deps.oauth_client_secret,
                    token=access,
        ...
        except Exception as exc:  # noqa: BLE001 — never block the cascade
            logger.warning("hr_dispatcher_obo_is_revoke_failed | reason=%s err=%r", reason, exc)
        ...
        *,
        ...
        if cached and not cached.is_near_expiry(now=now):
            ...
            try:
                ...
                tool_result = await self._maybe_attach_it_kit(tool, tool_result, request_id)
    ...
    async def _run_to_completion(
        ...
        *,
        ...
        try:
            ...
            # hr->it onboarding coordination: attach standard IT kit on the
            # cubicle summary (onboarding entry) and the final assignment.
            _kit_tool = {
                "get_cubicle_summary": "hr.cubicle_summary",
                "assign_cubicle": "hr.cubicle_assign",
            }.get(mcp_method)
            if _kit_tool:
                tool_result = await self._maybe_attach_it_kit(
                    _kit_tool, tool_result, request_id
                )
