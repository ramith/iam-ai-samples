...
def create_app(config: ITAgentConfig | None = None) -> FastAPI:
    ...
    # Instrument httpx (outbound HTTP spans + traceparent). Gated by
    # ENABLE_API_TRACES (default off) to keep traces quiet.
    ...
    # Server-side API request tracing (incoming HTTP). Off by default (noisy:
    # /healthz, A2A polls). ENABLE_API_TRACES=1 turns on traceparent extraction
    # so inbound A2A / peer calls nest under the caller's trace.
    if os.getenv("ENABLE_API_TRACES", "false").strip().lower() in ("1", "true", "yes"):
        try:
    ...
    # Peer coordination router: POST /a2a/peer/onboard-kit (non-CIBA, static).
    # Lets a sibling agent (hr_agent during onboarding) ask for the standard
    # new-hire IT kit without triggering a user consent flow.
    from it_agent.peer.router import PeerRouterDeps, build_peer_router
    app.include_router(
        build_peer_router(
            PeerRouterDeps(
                trusted_peer_callers=frozenset(
                    p for p in (cfg.trusted_peer_agents or ()) if p
                ) if hasattr(cfg, "trusted_peer_agents") else frozenset()
            )
        )
    )
