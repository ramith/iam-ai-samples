...
async def get_all_leave_requests(
    ...
    for req_id, req in store.leave_requests.items():
        if status and req["status"].lower() != status.lower():
            continue
        # Resolve a human-readable employee label, falling back through:
        # stored user_name → store.users join → sub local-part. Avoids the
        # bare "?" the composer renders when user_name was stored empty.
        employee_label = (req.get("user_name") or "").strip()
        if not employee_label:
            sub = req.get("user_sub", "")
            user_rec = store.users.get(sub, {})
            employee_label = (
                user_rec.get("name")
                or user_rec.get("username")
                or user_rec.get("email")
                or (sub.split("@", 1)[0] if sub else "")
                or sub
            )
        if employee_name and employee_name.lower() not in employee_label.lower():
            continue
        results.append({
            "request_id": req_id,
