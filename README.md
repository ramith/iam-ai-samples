...
| **WSO2 Identity Server** | `9443` | `wso2is` container | IdP. Local stack runs it as a container at `https://wso2is:9443` (self-signed cert), auto-provisioned by `scripts/provision-is.py`. |
...
- WSO2 IS itself runs **as a container** (`wso2is` + `mysql`) — no separate IS needed.
- A one-time hosts entry so the browser resolves the IS:
  `echo "127.0.0.1 wso2is" | sudo tee -a /etc/hosts`
- A **WSO2 Agent Manager (WSO2 AI Gateway) endpoint + key** for LLM routing (optional —
  without it the keyword router still works)
...
These hold IS client IDs/secrets, agent credentials, scopes, and the WSO2 AI Gateway settings.
**Do not commit them** — they contain live secrets. On the local stack they are **generated by
`scripts/provision-is.py`** (you don't hand-edit them); the full variable list + provenance is in
[`docs/architecture/fully-local-is-provisioning-spec.md`](docs/architecture/fully-local-is-provisioning-spec.md).
Headline vars:
...
./scripts/demo-down.sh           # stop + remove containers (keeps the volume)
```

`make demo-up` / `make demo-down` / `make demo-smoke` wrap the same scripts. Full design:
[`docs/architecture/fully-local-setup-plan.md`](docs/architecture/fully-local-setup-plan.md).
