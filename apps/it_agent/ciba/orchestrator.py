...
class ITDispatcher:
    ...
    async def revoke_jti(self, jti: str, user_sub: str, exp: float, reason: str) -> None:
        ...
        if cache_key is not None:
            ...
            if popped is not None:
                # Defence-in-depth: also revoke the OBO token at WSO2 IS.
    ...
    async def _revoke_obo_at_is(self, token, reason: str) -> None:  # type: ignore[no-untyped-def]
        """Best-effort revoke the OBO token (+ refresh) at WSO2 IS. Never raises."""
        try:
            ...
            logger.warning("it_dispatcher_obo_is_revoke_failed | reason=%s err=%r", reason, exc)
