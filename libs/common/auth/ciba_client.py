...
class CIBARequest:
    ...
    Attributes:
        ...
        requested_scope: The exact scope string sent to ``POST /oauth2/ciba``.
            Re-sent on token polling for compatibility with IS variants that
            derive final grants from poll-time scope intersection.
...
class CIBAClientConfig:
    ...
    Attributes:
        ...
        notification_channel: CIBA notification channel to request. Defaults
            to ``"external"`` so IS returns an ``auth_url`` that the SPA
            can open for user consent.
...
class CIBAClient:
    ...
    def _revoke_url(self) -> str:
        return f"{self.config.is_base_url}/oauth2/revoke"
    ...
    async def revoke_token(
        ...
        *,
        ...
        """Best-effort ``POST /oauth2/revoke`` for an OBO token (RFC 7009).

        Authenticated with the agent's OAuth client credentials (the client the
        token was issued to). This is layered on top of the local jti denylist
        as defence-in-depth so the token is also dead at the IdP (IS
        introspection reports it inactive), not just rejected locally.

        Never raises — revocation failure must not break the logout cascade.

        Args:
            oauth_client_id: The agent's OAuth Application client_id.
            oauth_client_secret: Corresponding client secret.
            token: The access (or refresh) token string to revoke.
            token_type_hint: ``"access_token"`` (default) or ``"refresh_token"``.

        Returns:
            ``True`` on a 2xx response (RFC 7009 returns 200 even for an
            already-invalid token); ``False`` on transport or non-2xx errors.
        """
        if not token:
            return False
        ...
        try:
            ...
            logger.warning("oauth2_revoke_transport_error | hint=%s err=%r", token_type_hint, exc)
            return False
        if resp.is_success:
            logger.info("oauth2_revoke_ok | hint=%s status=%d", token_type_hint, resp.status_code)
            return True
        logger.warning(
            "oauth2_revoke_non_2xx | hint=%s status=%d body=%s",
            token_type_hint,
            resp.status_code,
            resp.text[:200],
        )
        ...
        *,
        ...
        while _utc_now().timestamp() < deadline:
            ...
            try:
                ...
                if ciba_request.requested_scope:
                    token_form["scope"] = ciba_request.requested_scope
