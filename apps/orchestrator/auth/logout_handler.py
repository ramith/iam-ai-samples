...
class LogoutResult:
    ...
    Attributes:
        ...
        reason_label: ``"user_signed_out"`` always for UC-09. UC-10 will
            ...
            cascade behind an admin-triggered termination integration.
    """
...
class LogoutHandler:
        ...
        *,
        ...
        Args:
            ...
            reason: ``"user_signed_out"`` (UC-09) or ``"admin_terminated"``
                (admin-triggered future path). Surfaces in the audit chain and
                (3B.2)
    ...
    async def execute_for_user_sub(
        ...
        *,
        ...
        Used by admin-triggered termination integrations. There is no
        "initiating session" — the trigger came from
        ...
        Args:
            user_sub: User UUID to terminate.
            request_id: rid for log correlation.
        ...
        if not sessions:
            logger.info(
                "admin_terminate_cascade_no_sessions | rid=%s user_sub=%s reason=%s",
                request_id,
                user_sub,
                reason,
            )
    ...
    def _build_is_logout_url(self, id_token: str | None) -> str:
        ...
        Per source-code analysis, WSO2 IS processes logout participants using
        the RP-initiated logout request context. Our locked Q3 design always
        uses id_token_hint when present.
        ...
        return f"{self.config.is_browser_base_url}/oidc/logout?{urllib.parse.urlencode(params)}"
