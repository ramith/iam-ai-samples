...
class WSO2ISClientConfig:
    ...
    Attributes:
        base_url: Root URL of the WSO2 IS instance, e.g. ``https://localhost:9443``.
            No trailing slash.
    ...
    async def exchange_code(
        ...
        **body** as ``actor_token`` + ``actor_token_type``, NOT in the Authorization
        ...
        if actor_token is not None:
            ...
            # WSO2 expects JWT actor tokens to be tagged with the JWT token type.
            if actor_token.count(".") == 2:
                form_data["actor_token_type"] = "urn:ietf:params:oauth:token-type:jwt"
            else:
