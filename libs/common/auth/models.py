...
class JWTClaims:
    ...
    def effective_sub(self) -> str:
        """Return email (if present) instead of a bare UUID sub.

        Store lookups should use ``effective_sub`` rather than ``sub`` so that
        data keyed by ``email@example.com`` is found regardless of whether the
        OAuth app was configured to use ``userid`` or ``emailaddress`` as the
        OIDC subject at token issuance time.
        """
        if self.email:
            return self.email
        if self.sub and "@" not in self.sub and self.username:
            # sub is a UUID but username contains the email local-part — not
            # enough to reconstruct the full email, fall back to raw sub.
            pass
        return self.sub
