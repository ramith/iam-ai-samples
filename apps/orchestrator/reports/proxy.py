...
_HR_ADMIN_ROLE_MARKER = "HR Admin"
_ROLE_GATED_SCOPES = frozenset(
    {"hr_read_rest", "hr_approve_rest", "it_assets_read_rest"}
)


def _can_use_reports_scope(session: Session, required_scope: str) -> bool:
    token_a = session.token_a
    scope_string = (token_a.scope or "") if token_a is not None else ""
    if required_scope in scope_string.split():
        return True
    return required_scope in _ROLE_GATED_SCOPES and _HR_ADMIN_ROLE_MARKER in session.token_a_groups
...
async def forward_with_token_a(
    ...
    *,
    ...
    token_a = session.token_a
    if not _can_use_reports_scope(session, required_scope):
        scope_string = (token_a.scope or "") if token_a is not None else ""
        _logger.warning(
            "proxy_preflight_scope_denied | required=%s present=%r roles=%s session_id=%s",
            required_scope,
            scope_string,
            list(session.token_a_groups),
            session.session_id,
        )
        return _error(
            status_code=403,
