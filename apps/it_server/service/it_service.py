...
def get_my_assets(username: str = "", *, sub: str = "") -> dict:
    ...
    # Assets may be keyed by full email (seed) or by the email local-part
    # (issuance normalises email → local-part). Try every plausible key so the
    # sidebar resolves regardless of which path created the assignment.
    candidates: list[str] = []
    for cand in (username, sub):
    ...
    for cand in candidates:
        rows = store.get_assets_for_username(cand)
        if rows:
            matched_key = cand
            break

    logger.info(
        "[IT QUERY] get_my_assets username=%s sub=%s matched_key=%s returned=%d",
        username or "(unresolved)", sub or "(none)", matched_key or "(none)", len(rows),
    )
