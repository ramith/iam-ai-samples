...
from hr_agent.peer.it_peer_client import ITPeerClient
...
def create_app(config: HRAgentConfig | None = None) -> FastAPI:
    ...
    # Instrument httpx (outbound HTTP spans + traceparent for the hr->it chain).
    # Gated by ENABLE_API_TRACES (default off) to keep traces quiet.
    ...
    # so inbound A2A calls nest under the orchestrator's trace.
    if os.getenv("ENABLE_API_TRACES", "false").strip().lower() in ("1", "true", "yes"):
        try:
    ...
    # hr->it onboarding coordination peer client (non-CIBA). Base URL from
    # IT_AGENT_PEER_URL (compose injects http://it_agent:8002).
    _it_peer_client = ITPeerClient(
        base_url=os.getenv("IT_AGENT_PEER_URL", "http://it_agent:8002"),
    ...
    _dispatcher_for_router = HRDispatcher(
        deps=HRDispatcherDeps(
            ...
            ciba_scope=cfg.ciba_scope,
            max_poll_seconds=float(cfg.max_poll_seconds),
            it_peer_client=_it_peer_client,
        )
    )
