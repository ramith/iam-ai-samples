...
def build_it_mcp_router(deps: ITMcpToolRouterDeps) -> APIRouter:
    ...
    async def _validate_with_required_scope(
        ...
        *,
        ...
        Authorization must be enforced by IS-issued scopes at the resource
        server boundary; there is intentionally no scope downgrade path.
        """
        logger.debug(
    ...
    async def list_available_assets(
        ...
        try:
            await _validate_with_required_scope(
                ...
                rid=rid,
                tool_name="list_available_assets",
            )
    ...
    async def get_my_assets(
        ...
        result = it_service.get_my_assets(username or "", sub=claims.effective_sub)
    ...
    async def issue_asset(
        ...
        try:
            claims = await _validate_with_required_scope(
                token_str,
                required_scope="it_assets_write_rest",
                rid=rid,
                tool_name="issue_asset",
            )
        except (JWTValidationError, PeerTrustError, ScopeError) as exc:
            logger.warning(
