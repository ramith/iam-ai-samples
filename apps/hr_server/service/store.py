...
# Demo seed: pre-populated leave requests and a cubicle assignment so the
# Reports section shows meaningful data on a fresh start.
_SEED_LEAVE_REQUESTS: Dict[str, Dict] = {}

# Demo seed: pre-assigned cubicles for the demo users.
_SEED_CUBICLE_ASSIGNMENTS = [
    {
        "cubicle_id": "C-027",
        "username": "hradmin@example.com",
        "email": "hradmin@example.com",
        "sub": "hradmin@example.com",
    },
]
...
def reset_data() -> None:
    ...
    _demo_users = {
        "employee@example.com": {"name": "Employee", "sub": "employee@example.com", "username": "employee@example.com", "email": "employee@example.com"},
        "hradmin@example.com":  {"name": "HR Admin",  "sub": "hradmin@example.com",  "username": "hradmin@example.com",  "email": "hradmin@example.com"},
    }
    users = copy.deepcopy(_demo_users)
    leave_balances = {sub: default_balance() for sub in _demo_users}
    leave_requests = copy.deepcopy(_SEED_LEAVE_REQUESTS)
    leave_request_counter = len(_SEED_LEAVE_REQUESTS)
    cubicles = _seed_cubicles()
    # Apply seeded cubicle assignments
    for assignment in _SEED_CUBICLE_ASSIGNMENTS:
        from datetime import date
        for c in cubicles:
            if c["cubicle_id"] == assignment["cubicle_id"]:
                c["occupied"] = True
                c["assigned_to_username"] = assignment["username"]
                c["assigned_to_email"] = assignment["email"]
                c["assigned_to_sub"] = assignment["sub"]
                c["assigned_at"] = str(date.today())
                break
...
def ensure_user(
    ...
    *,
    ...
    elif sub not in leave_balances:
        # User was seeded without a balance — init now (lazy).
        leave_balances[sub] = default_balance()
        logger.info(
